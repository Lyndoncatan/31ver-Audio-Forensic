    ...
    try:
        ...
        if classification_path and os.path.exists(classification_path):
            try:
                ...
                # ---- STEP 2: Extract INDIVIDUAL clips per event ----
                # Instead of merging all events into one stem per category,
                # save each detected event as its own separate audio clip.
                # This allows the UI to show "HUMAN VOICE 1", "DOG BARK 1", etc.
                ...
                # Track per-category clip counts and sub-clips list
                category_counters = {}  # stem_key -> int counter
                stems_clips = {}  # stem_key -> [{label, url, time, confidence}, ...]
                count_generated = 0

                for event in events:
                    etype = event.get("type", "")
                    raw_label = event.get("rawLabel", etype)
                    ...
                    if target_stem and target_stem in pre_filtered:
                        ...
                        if seg_len > FADE_SAMPLES * 2:
                            ...
                            # Extract individual clip from pre-filtered audio
                            clip_audio = pre_filtered[target_stem][start_idx:end_idx] * envelope * scale

                            # Check if clip has meaningful audio
                            peak = np.max(np.abs(clip_audio))
                            if peak > 0.001:
                                # Normalize
                                normalized = clip_audio / max(peak, 1e-7)
                                normalized = np.clip(normalized, -1.0, 1.0)
                                int16_audio = (normalized * 32767).astype(np.int16)

                                # Increment counter for this category
                                if target_stem not in category_counters:
                                    category_counters[target_stem] = 0
                                category_counters[target_stem] += 1
                                clip_num = category_counters[target_stem]

                                # Save individual clip file
                                clip_filename = f"{target_stem}_{clip_num}.wav"
                                out_file = os.path.join(gen_dir, clip_filename)
                                wavfile.write(out_file, sr, int16_audio)

                                clip_url = f"/separated_audio/generated/{job_id}/{clip_filename}"

                                # Build a readable label like "DOG BARK 1"
                                clip_label = f"{raw_label} {clip_num}" if raw_label else f"{target_stem.upper()} {clip_num}"

                                if target_stem not in stems_clips:
                                    stems_clips[target_stem] = []
                                stems_clips[target_stem].append({
                                    "label": clip_label,
                                    "url": clip_url,
                                    "time": start_time,
                                ...
                                count_generated += 1
                                log(f"  Clip: {clip_filename} ({raw_label} @ {start_time}s, conf={confidence:.2f})")

                log(f"Generated {count_generated} individual event clips across {len(stems_clips)} categories.")

                # Merge into final_stems as sub-clip arrays
                for stem_key, clips in stems_clips.items():
                    final_stems[stem_key] = clips
            
            except Exception as e:
                log(f"Masking Exception: {str(e)}")

        # Also convert Demucs stems to the new list format for consistency
        for key in list(final_stems.keys()):
            if isinstance(final_stems[key], str):
                # Old format: single URL string -> wrap in list
                final_stems[key] = [{"label": key.upper(), "url": final_stems[key], "time": 0, "confidence": 1.0}]
